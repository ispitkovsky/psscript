...
function checkFile(fileName){
	var sizeStr = fileName.name.match(/(\d+)([x])(\d+)/);
	...
	var w = docRef.width.as('mm');
	var h = docRef.height.as('mm');
	...
	if ((Math.abs(w - width) > CONST_MARGIN) || (Math.abs(h - height) > CONST_MARGIN)) {
		...
		docRef.resizeCanvas(UnitValue((w+ext), "mm"), 
			UnitValue((h+ext), "mm"), AnchorPosition.MIDDLECENTER)

		docRef.flatten;

		SaveTIFF(fileName + '.tiff', docRef);
	}
}
